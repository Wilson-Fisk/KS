...
addonObject = addon('script.module.kingpinscrapers')
...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('kingpinscrapers_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('kingpinscrapers_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear kingpinscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('kingpinscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('kingpinscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.kingpinscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('kingpinScrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.kingpinscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.kingpinscrapers').getAddonInfo('version')
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.kingpinscrapers'
	try:
		removed_settings = []
		active_settings = []
